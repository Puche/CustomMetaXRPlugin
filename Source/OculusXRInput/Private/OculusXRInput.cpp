...
namespace OculusXRInput
{
	...
	ETrackingStatus FOculusXRInput::GetControllerTrackingStatus(const int32 ControllerIndex, const FName MotionSource) const
	{
		...
		if (!MotionSource.ToString().Contains("Left") && !MotionSource.ToString().Contains("Right"))
		{
			return TrackingStatus;
		}

		int32 DeviceID = MotionSource.ToString().Contains("Left") ? static_cast<int32>(EControllerHand::Left) : static_cast<int32>(EControllerHand::Right);
		...
		for (const FOculusControllerPair& ControllerPair : ControllerPairs)
		{
			if (ControllerPair.DeviceId == InDeviceId)
			{
				const FOculusTouchControllerState& ControllerState = ControllerPair.TouchControllerStates[DeviceID];
				...
				const FOculusHandControllerState& HandState = ControllerPair.HandControllerStates[DeviceID];
